...
CREATE OR REPLACE FUNCTION as_character(charactername TEXT)
  ...
  BEGIN
    -- TODO
    SET SESSION "session.is_system" = "FALSE";
    SET SESSION "session.user_id" = "01b45a47-8294-460b-96f6-6d8fc7c9ecde";
    SET SESSION "session.character_id" = "00000000-0000-0000-0000-000000000000";
    SET SESSION "session.tile_id" = "00000000-0000-0000-0000-000000000000";
    SET SESSION "session.settlement_id" = "00000000-0000-0000-0000-000000000000";
    RETURN TRUE;
  END;
  $$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION as_system()
  RETURNS BOOLEAN AS $$
  BEGIN
    SET SESSION "session.is_system" = "TRUE";
    SET SESSION "session.user_id" = "00000000-0000-0000-0000-000000000000";
    SET SESSION "session.character_id" = "00000000-0000-0000-0000-000000000000";
    SET SESSION "session.tile_id" = "00000000-0000-0000-0000-000000000000";
    SET SESSION "session.settlement_id" = "00000000-0000-0000-0000-000000000000";
    RETURN TRUE;
...
CREATE POLICY tiles_sec ON tiles
  USING (true)
  WITH CHECK (
    current_setting('session.is_system') = 'TRUE'
  );
...
CREATE POLICY tiles_items_sec ON tiles_items
  USING (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.tile_id')) = tile_id
  );
...
CREATE POLICY users_sec ON users
  USING (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.user_id')) = id
  );
...
CREATE POLICY characters_sec ON characters
  ...
  WITH CHECK (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.character_id')) = id
  );
...
CREATE POLICY users_characters_sec ON users_characters
  USING (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.user_id')) = user_id
  );
...
CREATE POLICY characters_skills_sec ON characters_skills
  USING (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.character_id')) = character_id
  );
...
CREATE POLICY settlements_sec ON settlements
  ...
  WITH CHECK (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.character_id')) = leader_id
  );
...
CREATE POLICY messages_sec ON messages
  USING (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.character_id')) = character_id
    OR uuid(current_setting('session.character_id')) = sender_id
  );
...
CREATE POLICY settlement_broadcasts_sec ON settlement_broadcasts
  USING (
    current_setting('session.is_system') = 'TRUE'
    OR uuid(current_setting('session.settlement_id')) = settlement_id
  );
